...
// GetPayidAndIntegratedAddr returns payid and address
func GetPayidAndIntegratedAddr(xmrCombineAddr string) (string, string, error) {
	addrCombine := strings.Split(xmrCombineAddr, ":")
	if len(addrCombine) != 2 {
		return "", "", fmt.Errorf("xmr address format wrong %s", xmrCombineAddr)
	}
	integratedAddr := addrCombine[0]
	payID := addrCombine[1]
	return payID, integratedAddr, nil
}
